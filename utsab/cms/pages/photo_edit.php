<?php
	...
	$sql="select * from  photo_master where sl='$id'";
	...
	if(isset($_POST['Submit']))
	{
		...
		$sql="UPDATE `photo_master` SET `photo_name` =  '$photo_name',`photo_desc` =  '$photo_description' where sl='$id'";
		mysqli_query($dbhandle,$sql);
		//echo $sql;
		 $mname =$_FILES["model_pro_pic"]["name"];
		 if($mname <> ''){
			 $path = "photo_upload/".date("dmYhs").$_FILES["model_pro_pic"]["name"];
			 ...
			 $sql="UPDATE `photo_master` SET `photo_pic` = '$mname' where sl='$id'";
	 		mysqli_query($dbhandle,$sql);
		 }		
...
?>
      <option value="<?php echo $rowx['pro_id']?> "  <?php if($rowx['pro_id']==$row['photo_type']){ ?>selected="selected"<?php } ?>><?php echo $rowx['product_name']?></option>
      <?php }} ?>
    </select></td>
  </tr>
   <tr>
